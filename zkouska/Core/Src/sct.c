...
void sct_value(uint16_t value, uint8_t led, uint8_t first) {
	uint32_t reg = 0;
	...
	if (first == 1) {
		reg |= reg_values[4][led]; // set barograph values
	}
	else {
		reg |= reg_values[3][led]; // set barograph values
	}
	sct_led(reg); // sends the value to the display - reg includes all useful numbers
}
